...
public abstract class SorterWithStatistics implements Sorter {
	...
	public SorterWithStatistics() {
		...
		completeTime = 0;
		sortType = "";
		sortType = this.getClass().getName();
		if(sortType.contains("MergeSort")) {
			sortType = "MergeSort";
		}
		else if (sortType.contains("QuickSort")) {
			sortType = "QuickSort";
		}
		else {
	...
	public void sort(String[] words, Comparator<String> comp) {
		...
		timer.stop();
		completeTime += timer.getElapsedTime();
	...
	public String getReport() {
		String str = ""; 
		str += sortType + ": " + getTotalTimeToSortWords();
		return str;
	}
}
