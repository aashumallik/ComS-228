...
public final class ResizableArrayBag<T> implements BagInterface<T> {
    ...
    public BagInterface<T> union(BagInterface<T> anotherBag) {
        ...
        BagInterface<T> unionList = new ResizableArrayBag<>();
        unionList.clear();

        for (int i = 0; i < numberOfEntries; i++) {

            unionList.add(this.bag[i]);

        }

        for (int i = 0; i < anotherBag.getCurrentSize(); i++) {

            unionList.add((T) anotherBag.toArray()[i]);
    ...
    public BagInterface<T> intersection(BagInterface<T> anotherBag) {
        ...
        ResizableArrayBag<T> a;
        ResizableArrayBag<T> b;

        if (anotherBagSize < currSize) {

            a = (ResizableArrayBag<T>) anotherBag;
            b = this;
        } else {
            a = this;
            b = (ResizableArrayBag<T>) anotherBag;
        }
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            for (int j = 0; j < other.length; j++) {
                ...
                if (other[j].equals(array[i]) && (status[j] == false)) {
    ...
    public BagInterface<T> difference(BagInterface<T> anotherBag) {
        ResizableArrayBag<T> newBag = new ResizableArrayBag<>((T[]) this.toArray());


        for (int j = 0; j < anotherBag.getCurrentSize(); j++) {

            newBag.remove((T) anotherBag.toArray()[j]);
